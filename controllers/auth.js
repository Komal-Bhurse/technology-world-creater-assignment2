...
const Register = async (req, res) => {
    try {
        ...
        const response = await SCPPartner.create({...body,password: hashedPassword });
...
const Login = async (req, res) => {
    ...
    try {
        ...
        if ( !mobile){
            return res.status(400).json({ status:"failed", message: 'Please fill the mobile' , data:""});
        }
        if(!password) {
            return res.status(400).json({ status:"failed", message: 'Please fill the password' , data:"" });
        }

        const user = await SCPPartner.findOne({ mobile });

        if(!user) {
            return res.status(404).json({ status: 'failed', message: 'Invalid mobile',data: '' });
        }

        const isPasswordMatch = await bcrypt.compare(password, user.password);

        if (!isPasswordMatch) {
          return res.status(404).json({ status: 'failed',  message: 'Invalid password',data: '' });
        }
        ...
        return res.status(500).json({ status: 'failed', message:'Unabled to login , please try again',data: ''})
...
const Logout = (req, res) => {
    try {
        ...
        return res.status(500).json({ status: 'faild', data: '', message:'Unabled to logout , please try again' })
    }
